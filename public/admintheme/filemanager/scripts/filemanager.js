...
var loadConfigFile = function (type) {
	...
	if(type == 'user') {
            var url = '../../admintheme/filemanager/scripts/filemanager.config.js';
	} else {
		var url = '../../admintheme/filemanager/scripts/filemanager.config.js.default'
	}
...
var lg = [];
$.ajax({
  url: '../../admintheme/filemanager/scripts/languages/'  + config.options.culture + '.js',
  async: false,
  dataType: 'json',
...
var getFolderInfo = function(path) {
	...
	$('#fileinfo').html('<img id="activity" src="../../admintheme/filemanager/images/wait30trans.gif" width="30" height="30" />');
...
$(function(){
	...
	/** load searchbox */
	if(config.options.searchBox === true)  {
		loadJS("../../admintheme/filemanager/scripts/filemanager.liveSearch.min.js");
	} else {
		$('#search').remove();
	}
