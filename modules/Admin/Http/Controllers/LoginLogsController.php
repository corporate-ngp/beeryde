    ...
    public function getData(Request $request)
    {
        ...
        return Datatables::of($loginLogs)
                ->addColumn('ids', function ($loginLogs) {
                    return '<input type="checkbox" name="ids[]" value="' . $loginLogs->id . '">';
                })
                ->addColumn('in_time', function ($loginLogs) {
                    return date('d M Y h:i A', strtotime($loginLogs->in_time));
                })
                ->addColumn('last_access_time', function ($loginLogs) {
                    return date('h:i A', strtotime($loginLogs->last_access_time));
                })
                ->addColumn('out_time', function ($loginLogs) {
                    return ($loginLogs->out_time === '0000-00-00 00:00:00') ? '0000-00-00 00:00:00' : date('d M Y h:i A', strtotime($loginLogs->out_time));
                })
                ...
                ->filter(function ($instance) use ($request) {
                    ...
                    if ($request->has('user_id')) {
                        $username = $this->userRepository->getUserIdByUsername($request->get('user_id'));
                        $instance->collection = $instance->collection->filter(function ($row) use ($username) {
                            return Str::equals($row['user_id'], $username) ? true : false;
                        });
                    }
                    if ($request->has('ip_address')) {
                        $instance->collection = $instance->collection->filter(function ($row) use ($request) {
                            return Str::equals($row['ip_address'], $request->get('ip_address')) ? true : false;
                        });
                    }
                    if ($request->has('login_in_time_from') && $request->has('login_in_time_to')) {
                        $instance->collection = $instance->collection->filter(function ($row) use ($request) {
                            ...
                            $fromDate = date('Y-m-d', strtotime($fromDateArray[0])) . ' ' . $fromDateArray[1];
                            $toDate = date('Y-m-d', strtotime($toDateArray[0])) . ' ' . $toDateArray[1];
                    ...
                    if ($request->has('access_time_from') && $request->has('access_time_to')) {
                        $instance->collection = $instance->collection->filter(function ($row) use ($request) {
                            $accessTime = strtotime(date('h:i:s A', strtotime($row['last_access_time'])));
